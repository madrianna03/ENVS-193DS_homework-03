...
# Convert variables to character to prevent them from being treated as numeric factors or continuous variables during plotting and reshaping.
# This step is important before using `pivot_longer()` to ensure consistent data structure.
df <- df %>%
  mutate(
...
# transformation allows all variables to be plotted together in faceted plots
df_long <- df %>%
  pivot_longer(
...
# Bin caffeine and sleep for clearer grouping
df_long <- df_long %>%
  mutate(
    value = case_when(
...
ggplot(df_long, aes(x = date, y = productivity, group = value, color = value)) +
  geom_line(linewidth = 1.1) + #line plot to show trend
  geom_point(size = 2) + #points to emphasize actual data entries
  facet_wrap(~ variable, scales = "free_x") + #one plot for each variable
  labs(
  ...
  theme_minimal(base_size = 14) + #clean and readable theme
